...
1-1. 슈퍼스칼라 구조를 설명해주세요
<details>
  <summary>정답</summary>
  슈퍼스칼라는 CPU 내부에 여러 개의 명령어 파이프라인을 두어 여러 명령어를 동시에 실행하는 구조입니다.

매 클럭 주기 동안 여러 명령어를 인출, 해석, 실행할 수 있습니다.
</details>

1-2. 이론적으로 파이프라인 개수가 많을수록 프로그램 처리 속도가 빨라지는데 실제로는 그에 비례해서 빨라지지는 않는 이유는 무엇인가요?
<details>
  <summary>정답</summary>
  데이터 위험, 제어 위험 등 파이프라인 위험으로 인해 동시에 실행할 수 없는 경우가 많기 때문입니다.

따라서 파이프라인 수를 늘려도 실제 성능 향상은 제한될 수 있습니다.
</details>

2-1. CISC가 명령어 파이프라이닝에 불리한 이유는 무엇인가요?
<details>
  <summary>정답</summary>
  가변 길이 명령어를 활용하기 때문에 명령어가 규격화되지 않고, 명령어 수행 시간도 일정하지 않아 파이프라이닝이 어렵습니다. 
</details>

2-2. ISA(명령어 집합 구조)가 달라지면 달라지는 부분들을 설명해주세요
<details>
  <summary>정답</summary>
  제어장치가 명령어를 해석하는 방식, 사용되는 레지스터의 종류와 개수, 메모리 관리 방법 등이 달라집니다.
  
  CPU 하드웨어 설계에도 큰 영향을 미칩니다.
</details>
